...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_sql = "SELECT ID FROM police WHERE ID = ?";
    $check_stmt = $conn->prepare($check_sql);
    if ($check_stmt === false) {
        $_SESSION['error_message'] = "Check prepare failed: " . $conn->error;
        header("Location: addpolice.php");
        exit();
    }
    ...
    if ($check_stmt->num_rows > 0) {
        $_SESSION['error_message'] = "Police ID already exists";
        $check_stmt->close();
        header("Location: addpolice.php");
        exit();
    }
    ...
    $sql = "INSERT INTO police (ID, name, crime_type, station_name, station_id, email, phone, address)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("ssssssss", $id, $name, $crime_type, $station_name, $station_id, $email, $phone, $address);

    if ($stmt->execute()) {
        // Log the activity for the dashboard
        $activity_type = "police";
        $description = "<span class=\"font-weight-bold\">New police officer</span> {$name} added to {$station_name}";
        ...
        $_SESSION['success_message'] = "Police officer added successfully!";
        header("Location: addpolice.php");
        exit();
    } else {
...
<body>
    ...
    <div class="d-flex">
        ...
        <div class="sidebar text-bg-dark d-flex flex-column flex-shrink-0 p-3">
            <a href="addpolice.php" class="d-flex align-items-center mb-3 mb-md-0 me-md-auto text-white text-decoration-none">
                <h4><i class="bi bi-shield-lock me-2"></i>Police Portal</h4>
            </a>
            <hr>
            <ul class="nav nav-pills flex-column mb-auto">
                <li class="nav-item"><a href="addpolice.php" class="nav-link text-white"><i class="bi bi-speedometer2 me-2"></i>Dashboard</a></li>
                <li class="mb-1">
                    ...
                    <div class="collapse" id="police-station-collapse">
                        <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small">
                            <li><a href="addpolicestation.php" class="nav-link text-white"><i class="bi bi-plus-circle me-2"></i>Add Station</a></li>
                ...
                <li class="mb-1">
                    <button class="btn btn-toggle d-inline-flex align-items-center rounded border-0 collapsed text-white w-100 active"
                        data-bs-toggle="collapse" data-bs-target="#police-collapse" aria-expanded="true">
                        <i class="bi bi-people me-2"></i>Police
                    </button>
                    <div class="collapse show" id="police-collapse">
                        <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small">
                            <li><a href="addpolice.php" class="nav-link text-white active"><i class="bi bi-person-plus me-2"></i>Add Police</a></li>
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-dark">
                    ...
                    <li><a class="dropdown-item" href="userlogin.php"><i class="bi bi-box-arrow-right me-2"></i>Sign out</a></li>
                </ul>
            </div>
        </div>
        ...
        <nav class="navbar navbar-dark bg-dark shadow-sm">
            <div class="container-fluid">
                <a href="addpolice.php" class="navbar-brand"><i class="bi bi-person-plus me-2"></i><strong>Add Police Officer</strong></a>
            </div>
        </nav>
            ...
            <div class="container mt-4">
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-dark text-white">
                        <h5 class="mb-0"><i class="bi bi-person-plus me-2"></i>Add Police Details</h5>
                    </div>
                    <div class="card-body">
                        <form id="addPoliceForm" action="addpolice.php" method="POST" novalidate>
                            <div class="row g-3">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <label for="ID" class="form-label">Police ID</label>
                                    <input type="text" name="ID" class="form-control" id="ID" required>
                                    <div class="invalid-feedback">Please provide a valid police ID</div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <label for="crime_type" class="form-label">Specialization (Crime Type)</label>
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="invalid-feedback">Please provide the officer's address</div>
                                </div>

                                <div class="col-12">
                                    <button type="submit" class="btn btn-dark px-4">
                                        <i class="bi bi-person-plus me-2"></i>Add Police Officer
                                    </button>
                                </div>
                            </div>
                        </form>
    ...
    <script>
        ...
        (function() {
            ...
            const form = document.getElementById('addPoliceForm');
